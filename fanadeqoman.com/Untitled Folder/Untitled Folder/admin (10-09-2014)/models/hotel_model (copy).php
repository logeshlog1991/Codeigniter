...
class Hotel_Model extends CI_Model {
	...
	*/
	...
	function get_booking_info()
	{
		...
		$select = "SELECT * FROM booking_info ORDER BY sno";
		$query = $this->db->query($select);
		if ( $query->num_rows > 0 )
		{ 
			return $query->result();
	...
	function get_cancelVocher($hotel_id,$sup_rate_id,$booking_id)
	{
		$this->db->select('*')
		 ...
		 if($query->num_rows()>0){
			...
			$update_price=0;$markup=$book_details->customer_finalcost;
			if($today_date < $startDatess){
				...
				$markup=$admin_details->admin_markup/100*$book_details->customer_finalcost;
				$update_price=$book_details->customer_finalcost-$markup;
			}
			$data = array('customer_canceltotal' => $update_price,'status'=>2,'markup'=>$markup);
