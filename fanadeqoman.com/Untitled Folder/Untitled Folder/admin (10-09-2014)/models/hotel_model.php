...
class Hotel_Model extends CI_Model {
    ...
    public function add_market($value)
	{
	  ...
	  /*
     * admin adding hotel accomdation
     */
    public function add_accommodation($value)
	{
		$sql="INSERT INTO sup_hotel_accommodation(accom_id, accom_name, status) VALUES('', '$value' ,'1')";
		$rs=$this->db->query($sql);
     ...
     * admin adding hotel accomdation
     */
    public function add_other_acc($value)
	{
		$sql="INSERT INTO sup_hotel_others(others_id, others_name, status) VALUES('', '$value' ,'1')";
		$rs=$this->db->query($sql);
	...
	*/
    ...
    public function add_hotel_details($hotel)
	{
		...
		$others='';
		if(isset($hotel['other_acc_name'])){
			$others=serialize($hotel['other_acc_name']);
		}
		$data = array(
			 'sup_id' => 'admin',
			 'market_name' =>$market_name ,
			 'accomodation_type' =>$hotel['acc_name'] ,
			 'other_accommodation' =>$others ,
			 'hotel_name' => $hotel['hotel_name'],	
			 'hotel_amenity'=>serialize($hotel['amenity_name']),   				 
			 'city' => $hotel['city_name'],
			 'city_country_name' => $hotel['hotel_country'],
			 ...
			 'res_fax' => $hotel['fax'],
			 'detail_location' => $hotel['hotel_location'],
	...
	*/
	...
	function B2C_bookingReports()
	{
		...
		$select = "SELECT * FROM booking_info where user_type!=1 ORDER BY sno";
		$query = $this->db->query($select);
		if ( $query->num_rows > 0 )
		{ 
			return $query->result();
	...
	function get_cancelVocher($hotel_id,$sup_rate_id,$booking_id)
	{
		$this->db->select('*')
		 ...
		 if($query->num_rows()>0){
			...
			$update_price=0;$markup=$book_details->customer_finalcost;$callcenter_markup=0;$gateway=0;
			if($today_date < $startDatess){
				...
				$this->db->select('*')
					  ...
					  ->where('usertype',1);
				$query1=$this->db->get();
				$admin_details=$query1->row();
				
				$this->db->select('charge')->from('payment_charge');
				$query3=$this->db->get();
				
				$gateway=$query3->row()->charge/100*$book_details->customer_finalcost;
				$rates=$book_details->customer_finalcost-$gateway;
				$markup=$admin_details->admin_markup/100*$rates;
				
				if(substr($book_name, 0, 4)=='FOCC'){
					$sql="SELECT * FROM callcenter_agents WHERE callagent_id='$book_name'";
					$query2=$this->db->query($sql);
					$callcenter_markup=$query2->row()->callcenter_markup/100*$book_details->customer_finalcost;
				}
				
				$update_price=$book_details->customer_finalcost-$markup-$callcenter_markup;
			}
			$data = array('customer_canceltotal' => $update_price,'status'=>2,'markup'=>$markup,'callcenter_markup'=>$callcenter_markup,'gateway'=>$gateway,'cancel_bookdate'=>date("Y-m-d h:i:sa"));
