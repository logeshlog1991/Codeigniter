...
class Hotel extends CI_Controller {
	...
	*/
	function booking_details()
	{
		...
		$data['type']="Confirm";
		$data['booking_info'] = $this->Hotel_Model->get_booking_info();
		//echo '<pre>';
	...
	*/
	function download()
	{
		...
		$data=array();
		$data[]=array('Sno','Booking ID','Booking Date','CheckIn','CheckOut','User_type','First Name','Last Name','E-mail ID','TelePhone','City','Hotel Name','Status','Currency','Purchase Price');
		
		$i=1;$grand=0;$bookings=0;
		if(isset($result) && $result!=''){
			foreach($result as $info){
					$data[]=array($i,$info->booking_id,$info->created_date,$info->customer_checkin,$info->customer_checkout,$info->customer_fname,$info->customer_lname,$info->user_type,$info->customer_email,$info->customer_phone,$info->customer_city,$info->hotel_name,$info->status,'$',$info->customer_finalcost);
