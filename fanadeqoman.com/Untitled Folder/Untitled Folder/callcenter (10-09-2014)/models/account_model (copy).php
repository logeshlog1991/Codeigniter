...
class Account_model extends CI_Model {
	...
	public function add_cancelamt($agent_id,$booking_id) {
		
		$this->db->select('customer_canceltotal')->from('booking_info')->where('booking_id',$booking_id);
		$query=$this->db->get();
	    $customer_canceltotal=$query->row()->customer_canceltotal;
		$refund_amt=$_SESSION['balance']+$customer_canceltotal;
		$data = array(
               'title' => $title,
               'name' => $name,
               'date' => $date
            );
	    $this->db->where('callcenter_id',$agent_id);
