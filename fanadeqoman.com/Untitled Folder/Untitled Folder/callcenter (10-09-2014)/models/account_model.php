...
class Account_model extends CI_Model {
	...
	public function my_booking($user_id) {
		...
		$sql="SELECT * FROM booking_info WHERE booking_name='$user_id' order by sno desc";
	...
	public function balance($user_id) {
		...
		$this->db->select('credit')->from('callcenter_credit')->where('status',1);
		$query=$this->db->get();
		if($query->num_rows()>0) {
			return $query->row();
		} 
		...
		$this->db->select('*')
		 ...
		 if($query->num_rows()>0){
			...
			if($today_date < $startDatess){
				...
				$sql="SELECT * FROM callcenter_agents WHERE callagent_id='$user_id'";
				$query2=$this->db->query($sql);
				
				$callcenter_markup=$query2->row()->callcenter_markup/100*$book_details->customer_finalcost;
			    $agent_balance=$book_details->customer_finalcost-$callcenter_markup;
			 
			    $refund_amt=$_SESSION['balance']+$agent_balance;
			...
			$data = array('customer_canceltotal' => $update_price,'status'=>2,'markup'=>$markup,'callcenter_markup'=>$callcenter_markup,'cancel_bookdate'=>date("Y-m-d h:i:sa"));
