						 ...
						 <?php 
								if($room_info!='') {
									for($i=0;$i<count($room_info[0]);$i++) {
													...
													$admarkup=($admintax!='')?$admintax->admin_markup/100*$rate:0;	
													$rate2=$rate+$admarkup; 						
													$tax=$this->Hotel_Model->get_tax($room_info[0][$i]->city_country_name);
													$markup=($tax!='')?$tax->country_markup/100*$rate2:0; 
													$rate3=$rate2+$markup; 
													$agent_id=$this->session->userdata('user_id');   
													$cal_tax=$this->Hotel_Model->get_agentinfo($agent_id);
													$cal_markup=($cal_tax!='')?$cal_tax->callcenter_markup/100*$rate3:0;
													$final_rate=round($rate3+$cal_markup);    
