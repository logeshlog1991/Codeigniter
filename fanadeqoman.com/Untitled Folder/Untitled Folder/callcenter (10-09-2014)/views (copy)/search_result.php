	...
	<?php $this->load->view('header_main'); ?>
		...
		<div class="container">
			...
			<div class="modifybox">
				<div class="row">
					<div class="col-lg-3">
						<div class="mofind">Find Your Hotel</div>
							<div class="input-group">
								<span class="input-group-addon"><i class=" icon-map-marker"></i></span>
								<input type="text" id="searchid" name="searchid"  class="form-control border-right" value="<?php echo $combination['cityname'];?>" placeholder="Region , Town , Hotel Name">
							</div>
			...
			<div class="row mt10">
				<!-- left side-->
					<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12 mt40column"> 
						<div class="well filtertxt">Filter By</div>
						<!--Accomodation Type-->        
						 <div>
							  <h3 class="heading3 togglehandle">Accomodation Type</span></h3>
							  <ul class="nav nav-list categories toggledata">
								 <?php 
										if(isset($acc_type) && $acc_type!=''){ 
											foreach($acc_type as $type){
								 ?> 
									<li><h4 class="font16"><input type="checkbox" name="acctype" class="acctype" onclick="return filter();" value="<?php echo $type->accom_name; ?>"><span class="ml15"><?php echo $type->accom_name; ?></span></h4></li>
								<?php
											}
										}
								 ?>
						...
						<!--Price Range -->        
						<div>
						  <h3 class="heading3 togglehandle"><span class="maintext">Price Range</span></h3>
						  <ul class="nav nav-list categories toggledata">
						...
						<!--Hotel Star Rating -->        
						<div>
						  <h3 class="heading3 togglehandle"><span class="maintext">Hotel Star Rating</span></h3>
						  <ul class="nav nav-list categories toggledata">
						...
						<!--Hotel Star Rating end-->  
						 <!--Location -->        
						<div>
						  <h3 class="heading3 togglehandle"><span class="maintext">Location</span></h3>
						  <ul class="nav nav-list categories toggledata">
							 <?php 
								  if(isset($location) && $location!=''){
									  ...
									  {
							?>
							<li><h4 class="font16"><input type="checkbox" name="location" class="location" onclick="return filter();" value="<?php echo $loc->detail_location; ?>"><span class="ml15"><?php echo $loc->detail_location; ?></span></h4></li>
							<?php
									  }
							      } 
							?>
						...
						<!--Location end-->  
						 <!--Amenities -->        
						<div>
						  <h3 class="heading3 togglehandle"><span class="maintext">Hotel Amenities</span></h3>
						  <ul class="nav nav-list categories toggledata">
							<?php 
								  if(isset($hotel_amen) && $hotel_amen!=''){
									  ...
									  {
							?>
							<li><h4 class="font16"><input type="checkbox" name="amenity" class="amenity" onclick="return filter();" value="<?php echo $amenity->amenity_name; ?>"><span class="ml15"><?php echo $amenity->amenity_name; ?></span></h4></li>
							<?php
									  }
							      } 
							?>
						...
						<!--Room Facilities -->        
						<div>
						  <h3 class="heading3 togglehandle"><span class="maintext">Room Facilities</span></h3>
						   <ul class="nav nav-list categories toggledata">
							   <?php
									if(isset($room_amen) && $room_amen!=''){
									  ...
									  {
								?>
									<li><h4 class="font16"><input type="checkbox" name="room_servces" class="room_servces" onclick="return filter();" value="<?php echo $rm_amen->facility_name; ?>"><span class="ml15"><?php echo $rm_amen->facility_name; ?></span></h4></li>
							<?php
									  }
							      } 
							?>
						...
						<!--Others-->        
						<div>
						  <h3 class="heading3 togglehandle"><span class="maintext">Others</span></h3>
						  <ul class="nav nav-list categories toggledata">
							  <?php
									if(isset($other_amen) && $other_amen!=''){
									  ...
									  {
								?>
								<li><h4 class="font16"><input type="checkbox" name="other_servces" class="other_servces" onclick="return filter();" value="<?php echo $amen_other->others_name; ?>"><span class="ml15"><?php echo $amen_other->others_name; ?></span></h4></li>
							<?php
									  }
							      } 
							?>
			...
			<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12 mt40column">
				...
				<div class="sorting well" style="background:#B5EBFF !important">
				  <form class=" form-inline pull-left col-lg-8 col-xs-12">
					<span class=" col-lg-4 col-xs-12 sorttxt">Sort By </span>
				  ...
				  <div class=" col-lg-4 col-xs-12  serachresultsc pull-right">Search Results : <span id="hotelCount"><?php if(isset($room_info[0]) && $room_info[0]!=''){  echo $room_info[1][0]->ffff; } else { echo '0'; }?></span></div>       
				</div>
					...
					<div class="list" id="list_result">
						...
						<!--1-->
						 <?php 
								if($room_info!='') {
									...
									$tax=$this->Hotel_Model->get_tax($room_info[0][$i]->city_country_name);
									$rate=$room_info[0][$i]->rate+$room_info[0][$i]->supplementary_charge_rate; 
									$markup=($tax!='')?($tax->country_markup/100*$rate):0;
						...
						?>
							<div class="searchhotel_box">
							  ...
							  <li class="clearfix HotelInfoBox" data-location="<?php echo $room_info[0][$i]->detail_location; ?>" data-acmptype="<?php echo $room_info[0][$i]->accomodation_type; ?>" data-star="<?php echo $room_info[0][$i]->star; ?>" data-hotelamenity="<?php if(isset($room_info[0][$i]->hotel_amenity)){ $val=''; $arr=unserialize($room_info[0][$i]->hotel_amenity);foreach($arr as $value){ $val=$val.$value.',';} $amen=rtrim($val,',');echo $amen; } ?>" data-roomservices="<?php if(isset($room_info[0][$i]->hotel_room_services)){ $val1=''; $arr1=unserialize($room_info[0][$i]->hotel_room_services);foreach($arr1 as $value1){ $val1=$val1.$value1.',';} $amen1=rtrim($val1,',');echo $amen1; } ?>" data-otherfac="<?php if(isset($room_info[0][$i]->other_accommodation)){ $val2=''; $arr2=unserialize($room_info[0][$i]->other_accommodation);foreach($arr2 as $value2){ $val2=$val2.$value2.',';} $amen2=rtrim($val2,',');echo $amen2; } ?>" data-price="<?php echo $final_rate; ?>" data-pricetype="<?php echo $price_type; ?>">
								<div class="thumbnail">
								  <div class="listclear">
									...
									<div class="col-lg-7 col-md-7 col-sm-12 col-xs-12" style="padding-left:0px;">
											<div class="address1"><div class="wid100"> <span class="text-center">
											<?php for($j=0;$j<$room_info[0][$i]->star;$j++) { ?>
											<i class="icon-star font12"></i>
											<?php }  if($j<5) {
											for($k=$j;$k<5;$k++) { ?>
											  <i class="icon-star-empty font12"></i>  
											<?php	} }?>
											</span></div><div class="mt5"><?php echo $room_info[0][$i]->address;?></div><div class="mt5"><?php echo $room_info[0][$i]->city.','.$room_info[0][$i]->city_country_name.'.';?></div></div>                       
															 </div>
										<!-- col-4-->
										  <div class="col-lg-5 col-md-5 col-sm-12 col-xs-12" style="padding-left:0px;">
										<div class="#mt25 pull-left text-left">
									   <div class="bluetxt"><?php echo $room_info[0][$i]->category_type;?></div>
									   <?php $var4=unserialize($room_info[0][$i]->hotel_room_services); for($k=0;$k<count($var4);$k++) { ?>
										<div class="mt5"><?php echo $var4[$k].',';?></div>
										<?php } 
												?>
									   </div>
										<div class="clearfix"></div>
											...
											<li class="col-lg-4 col-xs-7 mb0" title="<?php echo $i;?>">
												...
												<div class="tab-content  noborder dispn" id="tab<?php echo $i;?>" style=" border-top:1px dashed lightgray !important">
													...
													<div class="tab-pane" id="hotelfacilities<?php echo $i;?>">
														 ...
														 <div style="padding:30px;">
															 <?php $var = unserialize($room_info[0][$i]->hotel_amenity);
																for($k=0;$k<count($var);$k++) {?>
																	<div class="col-lg-4 text-left"> 
															 <p><i class="icon-paper-clip mr10"></i><?php echo $var[$k]; ?></p>
															  </div>
															 <?php } ?>
															 
																<?php $var1 = unserialize($room_info[0][$i]->other_accommodation);
																for($k=0;$k<count($var1);$k++) {?>
																	<div class="col-lg-4 text-left"> 
															 <p><i class="icon-paper-clip mr10"></i><?php echo $var1[$k]; ?></p>
															  </div>
															 <?php } ?>      
														</div>
													</div>
									...
									<div class="tab-pane" id="viewrates<?php echo $i;?>">
										...
										<div class="col-sm-12 col-xs-12">
											  <table class="table table-hover grayborder">
													...
													<tbody>
													  <tr>
														<td><?php echo $room_info[0][$i]->room_type;?><br><a data-toggle="modal" href="#myModal1"><h6>Room Details</h6></a></td>      
														<td><?php $max=explode('(',$room_info[0][$i]->room_type); $max_person=explode(',',$max[1]);
															for($j=0;$j<$max_person[0];$j++) {
															?><i class="icon-male"></i> <?php }?></td>
															<td><?php $var3=unserialize($room_info[0][$i]->hotel_room_services); for($k=0;$k<count($var3);$k++) { ?>
														<?php if($k>1){echo '<br>';} echo $var3[$k].',';}?></td>
														<td><?php if($room_info[0][$i]->status==1) { echo 'Available'; } else { echo 'Unavailable';} ?></td>
														<td><i class="icon-dollar"></i><?php $tax=$this->Hotel_Model->get_tax($room_info[0][$i]->city_country_name);
																$rate=$room_info[0][$i]->rate+$room_info[0][$i]->supplementary_charge_rate;
																$markup=($tax!='')?$tax->country_markup/100*$rate:0; 
																$final_rate=$rate+$markup;
														?>                  
