                     ...
                     <form name="form1" id="form1" class="form-horizontal add_hotel_form" enctype="multipart/form-data" action="<?php echo WEB_URL;?>hotel/edit_pricemanager_ad/<?php echo $rat_tac_details->sup_rate_tactics_id;?>" method="post">
                               <fieldset>
								...
								<div class="form-group">
								  ...
								  <div class="col-xs-12 col-md-4">
										<input id="roomTypes1" name="room_type1" placeholder="Room Category" readonly class="form-control input-md" type="text" value="<?php if(isset($rat_tac_details->hotel_room_type) && $rat_tac_details->hotel_room_type!=''){ echo $rat_tac_details->hotel_room_type; } ?>">
										<span style="color:red;" id="error_capacitytype"></span>
										<input id="roomTypes" name="room_type" placeholder="Room Category" readonly class="form-control input-md" type="hidden" value="<?php if(isset($rat_tac_details->sup_hotel_room_type_id) && $rat_tac_details->sup_hotel_room_type_id!=''){ echo $rat_tac_details->sup_hotel_room_type_id; } ?>">
								  </div>
								</div>
								<div class="form-group">
								  <label class="col-xs-12 col-md-3 control-label" for="textinput"><?php echo $this->lang->line('Room_Type'); ?></label>  
								  <div class="col-xs-12 col-md-4">
										<input id="roomTypeCapacity1" name="capacity_type1" placeholder="Room Type" readonly class="form-control input-md" type="text" value="<?php if(isset($rat_tac_details->capacity_id) && $rat_tac_details->capacity_id!=''){ $type=$this->Hotel_Model->get_roomType($rat_tac_details->capacity_id);echo $type->capacity_title;} ?>" >
										<input id="roomTypeCapacity" name="capacity_type" placeholder="Room Type" readonly class="form-control input-md" type="hidden" value="<?php if(isset($rat_tac_details->capacity_id) && $rat_tac_details->capacity_id!=''){ echo $rat_tac_details->capacity_id;} ?>" >
															...
															$.post( "<?php print WEB_URL ?>hotel/getDates", {mmsd:sd1, mmed:ed1, selectedDays:selectedDays},
															  function(data) { 
																if(data != ''){
																	...
																	$("#monthDates").append('<tr><td>'+data.dates[i]+'<input type="hidden" name="dates[]" value="'+day[0]+'"><input type="hidden" name="weekday[]" value="'+day[1]+'"></td><td><input name="price[]" type="text" size="5"/></td><td><input name="rack_price[]" type="text" size="5"/></td><td><input name="sell_price[]" type="text" size="5"/>    </td><td><input name="extra_bed_price[]" type="text" size="5"/></td><td><input name="avail[]" type="text" class="input-field" size="5"/></td><td><input name="adult[]" id="adult[]" type="text" size="5"/></td><td><input name="child[]" id="child[]" type="text" size="5"/></td><td><input name="child_price[]" type="text" size="5"/></td><td><input name="infant[]" type="text" size="5"/></td><td> &nbsp; </td></tr>');
																	}
																} 
															  }
															);
