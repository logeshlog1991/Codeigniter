					...
					<div class="list" id="list_result">
						 ...
						 <?php 
								if($room_info!='') {
									for($i=0;$i<count($room_info[0]);$i++) {
										...
										$payment_gateway=$this->Hotel_Model->payment_gateway();	
										$rateAftergateway=($payment_gateway/100)*$rate+$rate;
										
										$admintax=$this->Hotel_Model->get_admininfo();  
										$admarkup=($admintax!='')?($admintax->admin_markup/100)*$rateAftergateway+$rateAftergateway:0;	
										
										$tax=$this->Hotel_Model->get_tax($room_info[0][$i]->city_country_name);
										
										$final_rate1=($tax!='')?($tax->country_markup/100)*$admarkup+$admarkup:0; 
						...
						?>
							<div class="searchhotel_box">
							  ...
							  <li class="clearfix HotelInfoBox" data-location="<?php echo $room_info[0][$i]->detail_location; ?>" data-acmptype="<?php echo $room_info[0][$i]->accomodation_type; ?>" data-star="<?php echo $room_info[0][$i]->star; ?>" data-hotelamenity="<?php if(isset($room_info[0][$i]->hotel_amenity) && $room_info[0][$i]->hotel_amenity!=''){ $val=''; $arr=unserialize($room_info[0][$i]->hotel_amenity);foreach($arr as $value){ $val=$val.$value.',';} $amen=rtrim($val,',');echo $amen; } ?>" data-roomservices="<?php if(isset($room_info[0][$i]->hotel_room_services) && $room_info[0][$i]->hotel_room_services!=''){ $val1=''; $arr1=unserialize($room_info[0][$i]->hotel_room_services);foreach($arr1 as $value1){ $val1=$val1.$value1.',';} $amen1=rtrim($val1,',');echo $amen1; } ?>" data-otherfac="<?php if(isset($room_info[0][$i]->other_accommodation) && $room_info[0][$i]->other_accommodation!=''){ $val2=''; $arr2=unserialize($room_info[0][$i]->other_accommodation);foreach($arr2 as $value2){ $val2=$val2.$value2.',';} $amen2=rtrim($val2,',');echo $amen2; } ?>" data-price="<?php echo $final_rate; ?>" data-pricetype="<?php echo $price_type; ?>">
								<div class="thumbnail">
								  <div class="listclear">
									...
									<div class="col-lg-7 col-md-7 col-sm-12 col-xs-12" style="padding-left:0px;">
											...
											<?php	} }?>
											</span></div><div class="mt5"><?php echo $room_info[0][$i]->address;?></div><div class="mt5"><?php echo $room_info[0][$i]->city.','.$room_info[0][$i]->city_country_name.'.';?></div><div class="mt5"><?php echo '<span style="font-size:small;color:#00B0F0">Loaction:</span> '.$room_info[0][$i]->detail_location.'.';?></div>
											<div class="mt5"><?php echo '<span style="font-size:small;color:#00B0F0">Nearby Airport:</span> '.$room_info[0][$i]->nearby_airport.'.';?></div></div>                       
															 </div>
