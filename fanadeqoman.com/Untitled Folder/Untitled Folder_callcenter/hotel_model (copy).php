...
class Hotel_Model extends CI_Model {
    ...
    function get_rate($sup_rate_id) {
		...
		$this->db->select('sell_price')->from('sup_apart_maintain_month')->where('sup_rate_tactics_id',$sup_rate_id)->group_by('sup_rate_tactics_id');
		$query=$this->db->get();
		if($query->num_rows()>0) {
	...
	*/
	...
	function fetchingresult($cityname1='',$cin='',$cout='',$rooms1='',$adults='',$childs='',$val) {  $val = ($val!='')?$val:0;
	     ...
	     $select = "SELECT SQL_CALC_FOUND_ROWS t.*,m.*,r.*,c.*,h.*,A.*,B.* FROM sup_hotels h,sup_rate_tactics t,city_town A,country B,
					sup_apart_maintain_month m, sup_rate_cancel_policy c, sup_hotel_room_type r WHERE h.city='".$cityname1."' 
					AND h.status=1  AND t.hotel_id=h.sup_hotel_id AND A.c_no=h.city AND B.country_id=h.country AND 
	...
	function get_latest_hotels($value='') {
		$value = ($value!='')?$value:0;
		$sql="SELECT s.*,r.sup_rate_tactics_id,r.room_avail_date_from,r.room_avail_date_to,c.name as country_name,s.main_image from sup_hotels s INNER JOIN sup_rate_tactics r ON r.hotel_id=s.sup_hotel_id INNER JOIN country c ON c.country_id=s.country group by s.sup_hotel_id order by sup_hotel_id desc LIMIT $value,4 ";
	...
	function book_hotel($details,$user_id)
	{
		...
		$callcenter_markup=$this->Hotel_Model->get_callcenterMarkup($user_id);
		...
		if($i==$j){
			...
			$data = array(
				...
				'markup'  			 => $admarkup,
				'callcenter_markup'	 => $callcenter_markup,
	...
	function detect_cost($post,$balance,$user_id,$agent_tax) {
			if($balance >=$post['final_cost']) {
			  $balance_amt=$balance-$post['final_cost'];
	...
	function get_callcenterMarkup($user_id)
	{
		$this->db->select('*')
				 ->from('callcenter_agents')
