...
class Hotel_Model extends CI_Model {
	...
	*/
	...
	//to room type list
	 function room_list()
	 {
		$select = "SELECT A.*, B.*,A.status as main_status FROM sup_roomManager A LEFT JOIN sup_hotels B ON B.sup_hotel_id = A.hotel_id";
		$query=$this->db->query($select);
		if($query->num_rows() ==''){
			return '';
		}else{
	...
	*/
	public function edit_room_type_details($category,$hotel_id)
	{
	 ...
	 public function delete_room($room_id,$hotel_id)
	 {
		$sql="Delete from sup_roomManager where room_id='".$room_id."'";
		$res=$this->db->query($sql);
	 }
	...
	*/
	public function check_roomcategory($category_name)
	{
		$sql="SELECT * FROM  sup_roomCategory WHERE category_name='".$category_name."'";
		$res=$this->db->query($sql);
		if($res->num_rows() ==''){
			return 0;
		}else{
	...
	public function check_roomtype($category_name)
	{
		$sql="SELECT * FROM  sup_roomType WHERE roomtype_name='".$category_name."'";
		$res=$this->db->query($sql);
		if($res->num_rows() ==''){
			return 0;
		}else{
	...
	function add_roomcategory($room_details)
	{
			$data=array(
					'category_name'			=>	$room_details['room_category'],
					'category_nameA'		=>	$room_details['room_categoryA'],
				);
		$this->db->insert('sup_roomCategory',$data);
		$this->db->last_query();
		$id = $this->db->insert_id();
		if (!empty($id)) {
			return true;
	...
	function add_roomtype($room_details)
	{
			$data=array(
					'roomtype_name'			=>	$room_details['room_category'],
					'roomtype_nameA'		=>	$room_details['room_categoryA'],
				);
		$this->db->insert('sup_roomType',$data);
		$this->db->last_query();
		$id = $this->db->insert_id();
		if (!empty($id)) {
			return true;
	...
	function edit_category_ad($room_details,$cat_id)
	{
			$data=array(
					'category_name'			=>	$room_details['room_category'],
					'category_nameA'		=>	$room_details['room_categoryA'],
		...
		$where = "cat_id = $cat_id";
		$this->db->update('sup_roomCategory',$data,$where);
	}
	function edit_roomtype_ad1($room_details,$cat_id)
	{
			$data=array(
					'roomtype_name'			=>	$room_details['room_category'],
					'roomtype_nameA'		=>	$room_details['room_categoryA'],
		...
		$this->db->update('sup_roomType',$data,$where);
	}
	function delete_category($cat_id)
	{
		$sql="Delete from sup_roomCategory where cat_id='".$cat_id."'";
		$res=$this->db->query($sql);
		if ($res) {
			return true;
		}else{
	...
	function inactive_roomtype1($cat_id)
	{
		$data=array('status' =>	0);
		$where = "type_id = $cat_id";
		$this->db->update('sup_roomType',$data,$where);
	}
	function active_roomtype1($cat_id)
	{
		$data=array('status' =>	1);
		$where = "type_id = $cat_id";
		$this->db->update('sup_roomType',$data,$where);
	}
	function delete_type1($cat_id)
	{
		$sql="Delete from sup_roomType where type_id='".$cat_id."'";
		$res=$this->db->query($sql);
		if ($res) {
			return true;
		}else{
	...
	*  admin going to check room category is exit or not
	*/
	public function check_roomcategory1($room_category,$main_category)
	{
		$sql="SELECT * FROM sup_roomCategory WHERE category_name='".$room_category."'";
		$res=$this->db->query($sql);
		if($res->num_rows() ==''){
			return 0;
		}else{
			if($main_category!=$res->row()->category_name){
				return 1;
			}
		}
	}
	public function check_roomtype1($room_category,$main_category)
	{
		$sql="SELECT * FROM sup_roomType WHERE roomtype_name='".$room_category."'";
		$res=$this->db->query($sql);
		if($res->num_rows() ==''){
			return 0;
		}else{
			if($main_category!=$res->row()->roomtype_name){
				return 1;
			}
		}
	}
	...
	{
		$select = "SELECT r.*,c.* FROM sup_hotel_room_type r INNER JOIN sup_hotel_capacity c ON c.category_id=r.sup_hotel_room_type_id  WHERE c.capacity_id = '".$capacity_id."'";
		$query=$this->db->query($select);
		if($query->num_rows() ==''){
			return '';
		}else{
	...
	{
		$select = "SELECT room_id as sup_hotel_room_type_id,hotel_id,hotel_room_type FROM sup_roomManager where hotel_id = '".$hotel_id."' group by hotel_room_type";
		$query=$this->db->query($select);
	
		$select1 = "SELECT room_id as capacity_id,hotel_id,capacity_title,capacity,child_capacity FROM sup_roomManager where hotel_id = '".$hotel_id."' group by capacity_title";
	...
	*/
	...
	function add_pricemanager_ad($rack_price,$sell_price,$cancel_policy_descA,$child_policyA,$market_id,$cancel_policy_desc,$board_type,$hotel_id,$room_cate,$room_type,$child_policy,$cancel_policy_nights,$cancel_policy_percent,$cancel_policy_charge,$cancel_policy_from,$cancel_policy_to,$room_avail_date_from,$room_avail_date_to,$dates,$weekday,$price,$extra_bed_price,$avail,$aadult,$achild,$child_price,$infant,$fileimage,$fileimage1,$fileimage2,$fileimage3)
	{
				...
				for($i=0; $i<count($dates); $i++)
				{
					if(strtotime($dates[$i]) >= strtotime($sdate) && strtotime($dates[$i]) <= strtotime($edate))
					{
						...
						if($query->num_rows() ==''){
							...
							VALUES ( 'admin', '".$id."', '".$hotel_id."','".$room_cate."','".$room_type."', '".$date."', '".$weekday[$i]."', '".$day[0]."', '".$day[1]."', '".$day[2]."','".$price[$i]."', '".$sell_price[$i]."', '".$rack_price[$i]."','".$extra_bed_price[$i]."','".$avail[$i]."','".$aadult[$i]."', '', '', '', '".$child_price[$i]."', '', '1')";
							$query=$this->db->query($qry);
						}else{
							$upd="UPDATE sup_apart_maintain_month SET sup_rate_tactics_id='".$id."',week_day='".$weekday[$i]."',day='".$day[0]."',month='".$day[1]."',year='".$day[2]."',rate='".$price[$i]."',rack_price='".$sell_price[$i]."',sell_price='".$rack_price[$i]."',extra_bed_price='".$extra_bed_price[$i]."',available_rooms='".$avail[$i]."',num_mainrooms='".$aadult[$i]."',adult='',child='',child_charge='".$child_price[$i]."',infant='' WHERE date='".$date."' AND hotel_id='".$hotel_id."' AND room_cate='".$room_cate."'";
							$this->db->query($upd);
						}
					}
				}
		...
		$select = "SELECT MAX(smt.date) as Maxdate,MIN(smt.date) as Mindate FROM sup_rate_tactics a join sup_apart_maintain_month smt ON smt.sup_rate_tactics_id=a.sup_rate_tactics_id  where a.sup_rate_tactics_id = '$id' GROUP BY smt.room_cate";
		$query=$this->db->query($select);
	...
	function get_market($market_id)
	{
		...
		if($query->num_rows()>0){
			if($_SESSION['lang']=='english')
			{
				...
				return $query->row()->marketname_arab;
			}
		}else{
			return '';
		}
	...
	function get_payType($type_id)
	{
		...
		if($query->num_rows()>0){
			...
			if($_SESSION['lang']=='english'){
				...
				return $query->row()->payment_typeA;
			}
