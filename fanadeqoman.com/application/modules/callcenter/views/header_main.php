<div class="container">
		<div class="row " style=" margin:0;background:url(<?php echo WEB_DIR;?>public/img/headerbg1.jpg) repeat-x;height:35px;">
			<div class="col-lg-4"><h5><?php echo $this->lang->line('hotel_book'); ?></h5></div>
			<div class="col-lg-5">
				<div style="float:right;position:relative; top:10px;">
					<?php echo $this->lang->line('credit_bal'); ?> :
					<b>
						<?php 
							$markup='';$markup1='';
							$user_id=$this->session->userdata('user_id'); 
							$credit=$this->Account_model->balance($user_id); 
							$markup=$this->Account_model->callcenter_markup($user_id); 
							if($credit!='') { 
								$_SESSION['balance']=$credit->credit; echo $credit->credit.'$'; 
								$markup1=$markup;
							}else{ 
								echo '0$'; 
							}
					 ...
					 <nav class="subnav">
						  <div class="menurelative">
							...
							<li><a class="useridty"><i class="icon-user mr10"></i><?php echo $this->session->userdata('username'); ?></a>
								<div>
								  <ul>
									<li><a href="#" data-toggle="modal" data-target="#markup"><i class="icon-file-text mr10"></i><?php echo $this->lang->line('my_markup'); ?></a> </li>
									<li><a href="<?php echo WEB_URL;?>hotel_user/my_booking"><i class="icon-briefcase mr10"></i><?php echo $this->lang->line('My_Bookings'); ?></a> </li>
									<li><a href="<?php echo WEB_URL;?>index/logout"><i class="icon-signout mr10"></i><?php echo $this->lang->line('Sign_out'); ?></a> </li>
								  </ul>
								</div>
							  </li>
							</ul>
			...
			<div class="col-lg-3">	
				...
				<div class="language pull-right" style="margin-top:6px;">
					<span class="mr"></span>
					<span class="mr"><img  src="<?php echo WEB_DIR;?>public/img/icon/globe.PNG"></span>
				</div>
			</div>
			</div>
		</div><!-- row end-->    
...
<form id="formid" method="POST">
	<div class="modal fade" id="markup" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	  <div class="modal-dialog">
		<div class="modal-content" style=" margin-top:35%;">
		  <div class="modal-header">
			<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
			<h4 class="modal-title text-center" id="myModalLabel">Your Markup</h4>
		  </div>
		  <div class="modal-body">
			<div class="input-group col-lg-6 col-lg-offset-3">
				<span class="input-group-addon">Markup</span>
				<input type="text" class="form-control" id="markups" name="markups" value="<?php echo $markup1; ?>">
			</div>
		  </div>
		  <div class="modal-footer">
			 <button type="button" class="btn btn-warning col-lg-offset-5 col-lg-2" onclick="markup_update()">Update</button>
		  </div>
		</div>
	  </div>
	</div>
...
<script>
	function markup_update()
	{
		$.ajax({
			 url: "<?php echo WEB_URL;?>"+"hotel_user/update_markup",
			 type: 'post',
			 data:  {'mark_up':$("#markups").val()},
			 success: function(output) {
				 if(output==1){
					location.reload();
				 }
