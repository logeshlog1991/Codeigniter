...
class MY_index extends CI_Controller {
	...
	function sortbyresult() {
		...
		$cityname1=$this->session->userdata('cityname');
		$cin=$this->session->userdata('checkin');
		$cout=$this->session->userdata('checkout');
		$rooms1=$this->session->userdata('rooms');
		$adults=$this->session->userdata('adults_count');
		$childs=$this->session->userdata('childs_count');
		$data['room_info']=$this->Hotel_Model->fetchingresult($cityname1,$cin,$cout,$rooms1,$adults,$childs,$val);
		$data['pagi'] = $this->pagination($data['room_info'][1][0]->ffff);
	...
	function pagination($room){
		...
		$config['base_url'] = 'http://192.168.0.177/fanadeqoman.com/index.php/hotel/sortbyresult';
	...
	function change_language()
	{
		...
		if($post==''){
			...
			$this->session->set_userdata(array('lang'=>$post));
			$return=1;
		}
		echo $return;exit;
	}
