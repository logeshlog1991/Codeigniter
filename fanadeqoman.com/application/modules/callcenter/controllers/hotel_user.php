...
class Hotel_user extends CI_Controller {
	...
	function my_markup()
	{
		...
		$user_id=$this->session->userdata('user_id');
		$data['result']=$this->Account_model->get_markup($user_id);
		$this->load->view('my_markup',$data);
	}
	function update_markup()
	{
		$user_id=$this->session->userdata('user_id');
		$data['result']=$this->Account_model->update_markup($this->input->post('mark_up'),$user_id);
